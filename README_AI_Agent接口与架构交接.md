...
- AI 不得执行 `WAIVE` 或批准整单。任一高置信度未通过项可由 Backend 自动退回；全部单项通过后，整单仍等待会计确认；
...
`scope` 只能是 `CURRENT | HISTORY`。`search_history` 长度必须等于 `turn`，最多三轮搜索。
...
- 最多三轮。仍无法判断时应返回 `ESCALATE`，不能继续搜索或猜测结论。
...
- `AUTO_REVIEW`：仅在 schema、evidence、金额和置信度全部通过时自动执行单项结论；
- `SATISFY` 达到请求的满足阈值后，单项进入 `SATISFIED`；
- `REQUEST_ACTION` 达到退回阈值后，单项进入 `NEEDS_ACTION`，整单进入 `CHANGES_REQUESTED`；
- 低置信度、`ESCALATE`、金额不一致、非法 evidence 或非法输出转人工；
...
模型不要根据阈值自行隐藏不确定结果。应始终返回真实校准后的 confidence，由 Backend 应用每个请求自己的阈值。
